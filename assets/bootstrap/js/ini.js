...
$(document).ready(function () {
    ...
    $("body").on('click', '.submit-ajax-button', function () {
        ...
        if (button.hasClass('del')) {
            if (!confirm('Está seguro de eliminar este item?', 'Confirmar acción')) {
                return false;
            } else {
                url = button.val();
                tipo = 2;
            ...
            if (!confirm('Está seguro de realizar esta acción?', 'Confirmar acción')) {
                return false;
            } else {
                if (!form.validate()) {
                    return false;
        ...
        } else if (button.hasClass('cpeconfirm')) {
            if (!confirm('Está seguro de emitir este comprobante?', 'Confirmar acción')) {
                return false;
            } else {
                if (!form.validate()) {
                    return false;
...
function AjaxPopupModal(id, title, url, params)
{
    ...
    // Cargando
    $("#" + id).find('.modal-body').html('<blink>Estamos cargando el formulario ..</blink>');
