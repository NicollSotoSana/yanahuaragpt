...
class EgresosModel extends CI_Model
{
	public function Listar()
	{
		...
		$this->db->where($where);
		$this->jqgridmodel->Config($this->db->SELECT('COUNT(*) Total FROM egresos')->get()->row()->Total);
	...
	public function getEgreso($id){
		return $this->db->where("id_egreso", $id)->get("egresos")->row();
	}
	...
	public function Guardar($data){
		...
		if(!isset($data["id_compra"])){
			...
			$this->responsemodel->href   = 'almacen/nuevaCompra/'.$data["id_compra"];
		}
