...
class ReporteModel extends CI_Model
{
		...
		$sql = "
			...
			(
				SELECT
					...
					SUM(CASE WHEN ComprobanteTipo_id = 4 THEN Total ELSE 0 END) Nota,
	...
	public function ReporteComprobantesDetalle($fecha)
	{
		$sql = "
			SELECT 
				cd.ProductoNombre Nombre,
	...
	public function ReporteDiarioCaja($mes, $anio)
	{
		...
		for($i = 1; $i <=$dias; $i++)
		{
			$sql = "
				SELECT 
					*,
					Boleta + Factura Vendido
				FROM 
				(
					SELECT
						FechaEmitido,
						SUM(CASE WHEN adelanto > 0 THEN adelanto ELSE Total END) Boleta,
						SUM(CASE WHEN adelanto > 0 THEN adelanto ELSE Total END) Factura
					FROM comprobante
					WHERE MONTH(FechaEmitido) = $mes
					AND YEAR(FechaEmitido) = $anio
					AND DAY(FechaEmitido) = $i
					AND Empresa_id = " . $this->user->Empresa_id . "
					AND ComprobanteTipo_id IN (2,3)
					AND Estado = 2
					AND Correlativo IS NOT NULL
					GROUP BY FechaEmitido
					ORDER BY FechaEmitido DESC
			...
			$t_ventas = (isset($ventas[0]->Vendido)) ? $ventas[0]->Vendido : "0.00";

			$egr = $this->db->query('SELECT IFNULL(SUM(monto_egreso), 0.00) total_egreso FROM egresos WHERE Empresa_id = ' . $this->user->Empresa_id . ' and MONTH(fecha) = '.$mes.' AND YEAR(fecha) = '.$anio.' AND DAY(fecha) = '.$i.' GROUP BY fecha')->result();
			$t_egresos = (isset($egr[0]->total_egreso)) ? $egr[0]->total_egreso : "0.00";

			$comp = $this->db->query('SELECT IFNULL(SUM(monto), 0.00) total_compra FROM depositos WHERE Empresa_id = ' . $this->user->Empresa_id . ' and MONTH(fecha) = '.$mes.' AND YEAR(fecha) = '.$anio.' AND DAY(fecha) = '.$i.' GROUP BY fecha')->result();
			$t_compras = (isset($comp[0]->total_compra)) ? $comp[0]->total_compra : "0.00";
			$retorno[$i] = array("Fecha" => $anio.'-'.$mes.'-'.$i,"Vendido" => $t_ventas, "Egresos" => $t_egresos, "Compras" => $t_compras);
		}
