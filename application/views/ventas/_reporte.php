...
<div id="dvFiltro" class="row">
	...
	<div class="col-md-3">
		...
		<?php endif; ?>
		<?php if($tipo==4 || $tipo==5 || $tipo==7 || $tipo==8): ?>
		<div id="dvMes" class="form-group">
	...
	<div class="col-md-2">
		<?php if($tipo == 1 || $tipo == 2 || $tipo == 4 || $tipo == 5 || $tipo == 6 || $tipo == 7 || $tipo==8): ?>
			...
			<table class="table report">
				...
				<tbody>
					<tr>
						...
						<th style="width:140px;" class="text-right">Nota</th>
					...
					">
						<th class="text-right" style="width:60px;">
							...
							<?php }else if($tipo == 3){ ?>
								<a class="aReporteDiarioDetalle" title="Haga click para ver un resumen de los productos vendidos" href="#" data-fecha="<?php echo $r->FechaEmitido; ?>">
									<?php $fv = explode("/", $r->FechaEmitido); echo $fv[0];?>
								</a>
							<?php }else {?>
								<?php echo DateFormat(toDate($r->FechaEmitido), $tipo); ?>
							<?php } ?>
						...
						<td class="text-right"><?php echo $this->conf->Moneda_id; ?> <?php echo number_format($r->Nota, 2); $TNota += $r->Nota;  ?></td>
				...
				<tfoot>
					<tr>
						...
						<td class="text-right"><?php echo $this->conf->Moneda_id; ?> <b><?php echo number_format($TNota, 2); ?></b></td>
			...
			<table class="table report">
				...
				<tbody>
					<tr>
						...
						<th style="width:140px;" class="text-right">Ingresos</th>
						<th style="width:140px;" class="text-right">Egresos</th>
						<th style="width:140px;" class="text-right">Nota</th>
						<th style="width:140px;" class="text-right">Compras</th>
					</tr>
					<?php if(count($reporte)==0): ?>
					<tr>
						<th colspan="6" class="text-center">No hay resultados para este criterio</th>
					...
					<?php endif; ?>
					<?php foreach($reporte as $r): ?>
					<tr class="<?php echo $r["Fecha"] == date('d/m/Y') ? 'today' : ''; ?>
					">
						<td class="text-right" style="width:60px;">
							<?php echo date('d/m/y', strtotime($r["Fecha"])); ?>
							
						</td>
						<td class="text-right"><?php echo $this->conf->Moneda_id; ?> <?php echo number_format($r["Vendido"], 2); $T_ingresos += $r["Vendido"]; ?></td>
						<td class="text-right"><?php echo $this->conf->Moneda_id; ?> <?php echo number_format($r["Egresos"], 2); $T_egresos += $r["Egresos"];  ?></td>
						<td class="text-right"><?php echo $this->conf->Moneda_id; ?> <?php echo number_format($r["Compras"], 2); $T_compras += $r["Compras"]; ?></td>
					</tr>
					<?php endforeach; ?>
				</tbody>
				<tfoot>
					<tr class="">
						<th class="total text-right bg-earned-total">Total</th>
						<td class="total text-right bg-earned-total"><?php echo $this->conf->Moneda_id; ?> <b><?php echo number_format($T_ingresos, 2); ?></b></td>
						<td class="total text-right bg-earned-total"><?php echo $this->conf->Moneda_id; ?> <b><?php echo number_format($T_egresos, 2); ?></b></td>
						<td class="total text-right bg-earned-total"><?php echo $this->conf->Moneda_id; ?> <b><?php echo number_format($T_compras, 2); ?></b></td>
	...
	<?php }else if($tipo == 9){ ?>
		<div class="col-md-3">
			<div class="form-group">
				<label>Desde:</label>
				<input type="date" name="cinicio" id="dinicio" class="form-control" value="<?php echo date('Y-m-d');?>">
			</div>
		</div>
		<div class="col-md-3">
			<div class="form-group">
				<label>Hasta:</label>
				<input type="date" name="cfin" id="dfin" class="form-control" value="<?php echo date('Y-m-d');?>">
			</div>
		</div>
		<div class="col-md-3">
			<br/>
			<button onclick="getReporte()" class="btn btn-success"><b>Descargar Excel</b></button>
		</div>
